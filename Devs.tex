...
\begin{enumerate}
    \item \textbf{Puerto unificado de alarma}: Se unifica la recepción en un solo
    puerto \texttt{alarma} con valores enumerados \texttt{\{baja, media, critica\}},
    simplificando el acoplamiento con el controlador.

    \item \textbf{Estados transitorios} ($ta = 0$): Al recibir cualquier alarma,
    $\sigma = 0$ fuerza la ejecución inmediata de $\lambda$ y $\delta_{\mathit{int}}$.

    \item \textbf{Lógica de repetición}: Tras 30 segundos sin confirmación,
    $\delta_{\mathit{int}}$ pasa a la fase \texttt{REPETIENDO} con ciclos de
    $\sigma = 10$ segundos, cumpliendo el requisito temporal especificado.

    \item \textbf{Gestión de confirmación}: Al recibir \texttt{confirmacionEnfermero}
    con alarma crítica activa, el modelo pasa a reposo ($\sigma = \infty$),
    deteniendo el ciclo de repetición.
\end{enumerate}
\end{document}
