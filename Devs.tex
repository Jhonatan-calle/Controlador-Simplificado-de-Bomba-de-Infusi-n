...
\dext{}((tipoAlarma, fase, \cmlsigma{}), e, (puerto, valor)) \kw{is}
    \kw{defcases}
        fase = NOTIFICANDO; \cmlsigma{} = 0.0;
            \kw{if} (puerto = alarma \cmlwedge{} valor = critica) tipoAlarma = CRITICA;
            \kw{if} (puerto = alarma \cmlwedge{} valor = media)   tipoAlarma = MEDIA;
            \kw{if} (puerto = alarma \cmlwedge{} valor = baja)    tipoAlarma = BAJA;
...
\begin{enumerate}
    ...
    de $\lambda$ seguida de $\delta_{\mathit{int}}$ para emitir el aviso sin demora.
    ...
    y establece $\sigma = \infty$, deteniendo cualquier ciclo de repetición activo.
