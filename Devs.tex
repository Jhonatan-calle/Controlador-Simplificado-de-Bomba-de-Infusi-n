...
\kw{end Y}
\dext{}((caudalMedido, caudalActuador, \cmlsigma{}), e, (puerto, valor)) \kw{is}
    caudalActuador = valor;
    \cmlsigma{} = \cmlsigma{} - e;
        \kw{if} (puerto = caudalActual)
\kw{end} \dext{}
\dint{}(caudalMedido, caudalActuador, \cmlsigma{}) \kw{is}
    \cmlcomment{-- Si hay valor del actuador lo usa, sino genera internamente}
    caudalMedido = next\_caudal\_logic(caudalActuador);
    \cmlsigma{}          = 1.0;
\kw{end} \dint{}
\clambda{}(caudalMedido, caudalActuador, \cmlsigma{}) \kw{is}
    sensorFlujo = caudalMedido;
\kw{end} \clambda{}
\cmlta{}(caudalMedido, caudalActuador, \cmlsigma{}) \kw{is}
    \cmlsigma{};
\kw{end} \cmlta{}
\kw{end atomic}
\end{cmldevs}
...
\begin{enumerate}
    ...
    en \texttt{caudalActuador} y mantiene el cronograma con $\sigma = \sigma - e$.
    En la próxima transición interna, \texttt{next\_caudal\_logic(caudalActuador)}
    utiliza ese valor para generar la medición.

    \item \textbf{Estado inicial} ($S$): Se asume \texttt{caudalMedido} $= 0.0$,
    \texttt{caudalActuador} $= 0.0$ y $\sigma = 1.0$.
\end{enumerate}
