...
La consigna establece que el criterio exacto para escalar de \texttt{alarmaMedia} a
\texttt{alarmaCritica} debe ser definido por cada grupo. En este trabajo se optó por
incorporar un contador de alarmas medias (\texttt{contadorMedia}) en el estado del
modelo. Cuando el controlador emite tres alarmas medias consecutivas sin que el
desvío de caudal sea corregido, el módulo interpreta que el sistema no ha logrado
estabilizarse y escala automáticamente a \texttt{alarmaCritica}, reseteando el
contador. Este criterio permite una escalada objetiva y trazable sin requerir
intervención externa adicional.
\begin{center}
\begin{cmldevs}
...
\dext{}((tipoAlarma, fase, contadorMedia, \cmlsigma{}), e, (puerto, valor)) \kw{is}
    \kw{defcases}
        ...
        tipoAlarma = CRITICA; fase = NOTIFICANDO;
        contadorMedia = 0; \cmlsigma{} = 0.0;
            \kw{if} (puerto \cmleq{} alarma \cmlwedge{} valor \cmleq{} media \cmlwedge{} contadorMedia \cmlplus{} 1 \cmlgeq{} 3)
        tipoAlarma = MEDIA; fase = NOTIFICANDO; 
        contadorMedia = contadorMedia + 1; \cmlsigma{} = 0.0;
        ...
        tipoAlarma = NINGUNA; fase = OCIOSO; 
        contadorMedia = 0; \cmlsigma{} = infinity;
...
\begin{enumerate}
    ...
    \item \textbf{Gestión de confirmación}: Si llega \texttt{confirmacionEnfermero} 
      con \texttt{tipoAlarma = CRITICA}, la $\delta_{\mathit{ext}}$ limpia el 
      estado y resetea \texttt{contadorMedia} a 0, deteniendo cualquier ciclo de 
      repetición. Si no hay alarma crítica activa, la confirmación se ignora 
      manteniendo elronograma con $\sigma = \sigma - e$.
